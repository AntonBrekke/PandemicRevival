# PandemicRevival
Sterile_caller.py
- Only one function: `call`. Is used in `find_y.py`.
- Uses `Gamma_X_new` from `vector_mediator.py` to get total decay rate for X, and classes `TimeTempRelation` to relate SM temperatur and time, and `Pandemolator` for solving Boltzmann equation from `pandemolator.py`.
...
- The spin-statistics-less cousin of C_res_vector, making simplifications for when spin-statistics does not matter. Is not fully updated, is used un `C_therm_kd` in `sterile_caller.py`. 
