...
sys.path.insert(0, currentdir+'/2to4')
...
filename = f'rates_md_{m_d:.4e}_mphi_{m_phi:.4e}_sin22th_{sin2_2th:.4e}_y_{y:.4e}_2to4.dat'
if not False:# os.path.isfile('./' + filename):
    ...
    C_2p_4p = np.array([2.*sigma_2to4.C2to4(1., m_phi, T_d, xi_phi) for T_d, xi_phi in zip(T_d_grid, xi_phi_grid)])

    np.savetxt(filename, np.column_stack((m_d/T_nu_grid, H_grid, C_p_dd/n_d_grid, C_dd_p/n_d_grid, C_p_da/n_d_grid, C_da_p/n_d_grid, C_p_aa/n_d_grid, C_aa_p/n_d_grid, C_pp_dd/n_d_grid, C_dd_pp/n_d_grid, C_dd_dd/n_d_grid, C_da_dd/n_d_grid, C_dd_da/n_d_grid, C_aa_dd/n_d_grid, C_dd_aa/n_d_grid, C_2p_4p/(n_d_grid+2.*n_phi_grid))))
...
plt.loglog(x_grid, 1e6*H, color='#83781B', ls='-')
# plt.loglog(x_grid, 1e24*Hnphi, color='darkorange', ls='-')
# plt.loglog(x_grid, 1e24*Hnnu, color='#A300CC', ls='-')
plt.loglog(x_grid, 1e6*C_dd_p, color='#114B5F', ls='-')
plt.loglog(x_grid, 1e6*C_dd_p*1.64e-13/1.42e-19, color='#186E8B', ls='-')
plt.loglog(x_grid, 1e6*C_da_p, color='#458751', ls='-')
...
plt.loglog(x_grid, 1e6*C_dd_pp, color='#95190C', ls='-')
plt.loglog(x_grid, 1e6*C_pp_dd, color='#D02411', ls='-')
plt.loglog(x_grid, 1e6*C_dd_dd, color='#114B5F', ls=':')
plt.loglog(x_grid, 1e6*C_2p_4p, color='darkorange', ls='-')
# plt.loglog(x_grid, 1e6*C_da_dd, color='#458751', ls=':')
